package com.andeptrai.doantn_admin_restaurant;

import java.io.Serializable;

public class InfoAdminCurr implements Serializable {

    public static int currentId = 1;
    public static String currentUsername = "an01470147";
    public static String currentPwd = "an01470147";
    public static String currentPhone = "an01470147";
    public static String currentEmail = "abc@abc.com";
    public static String currentName = "Nguyen Van A";
    public static String currentAddress = "so 1 hoang cau, hoan kiem, ha noi";
    public static String list_care_res = "";
    public static int checkAdmin = 1;
}
